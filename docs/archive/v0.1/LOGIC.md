...
### 1.1 パス解決ロジック
- **`STEAM_INSTALL_PATH`**: Windows形式 (`C:\...`) または Linux形式 (`/mnt/...`) を許容。内部で `utils.ensure_path` を通じて Linuxパスに正規化される。
...
「曲順・曲名という絶対的な構造は Steam ストア公式データを正本とする」方針に基づき、システムは以下の順序で情報を補完する。
1. **構造情報 (トラック番号・ディスク番号・曲名)**:
   - 最優先: Steam公式データ (`STEAM_STORE`, `PICS_API`)
   - フォールバック: LLMが指紋使用を指示した場合、またはSteamデータが空の場合のみ `FINGERPRINT` または `MBZ` から補完。
2. **付加情報 (アーティスト・年・レーベル)**:
   - 最優先: `MBZ` (MusicBrainz)
...
- **構築責務の集約**: `LocalProcessor` は個別設定値をその場で組み立てず、`Config.build_mbz_scoring_config()` と `Config.build_llm_organizer_kwargs()` を通じて依存オブジェクト初期化に必要な設定群を取得する。

- **`LLM_LIMIT_RPM` / `LLM_LIMIT_TPM`**: クラウドAPIの毎分リクエスト/トークン制限。超過による429エラーを防ぐためのスレッド制御やチャンク算出に使用。
- **`LLM_CLOUD_MAX_TOKENS`**: クラウドAPIのモデル別絶対出力限界（例: 8192）。動的チャンクの算出に使用。
- **`LLM_OLLAMA_NUM_CTX`**: OLLAMAの推論に割り当てる最大コンテキスト長（VRAM算出の基準）。
- **`LLM_OLLAMA_NUM_PREDICT`**: OLLAMAの出力に対する「信頼度の天井」（デフォルト: `4096`）。実際のAPI送信時は `-1` が付与されるが、この設定値はチャンクサイズの動的算出の数学的上限として使用される。
- **`LLM_CHUNK_ADAPTIVE`**: 出力予算とTPMに応じたチャンク自動最大化（固定値の無視）を有効化（デフォルト: `true`）。
- **`LLM_CHUNK_SIZE_VIRTUAL` / `METADATA_*`**: 上記機能が無効（`false`）にされた場合のみ使われる固定チャンクのフォールバック値。
- **`LLM_CHUNK_OUTPUT_TOKENS_PER_TRACK`**: 1トラック当たりの出力見積（デフォルト: `180`）。
- **`LLM_CHUNK_OUTPUT_SAFETY_RATIO`**: 出力上限に対する安全率（デフォルト: `0.75`）。
...
6.  **音声指紋（AcoustID）による補完**:
    ...
    - **全曲スキャン（デフォルト設定）**: 現在は精度の高さを最優先とし、すべてのトラックを対象にAcoustIDによる数学的確定スキャンを行います。詳細は 3.3項参照。
7.  **論理IDの決定**:
...
- **Phase 1.5 (Coherence Map-Reduce)**:
  ...
  - 数百曲のフルデータをそのまま投げず、全体から節目となる曲のみを抽出した「Skeleton（骨組み）」を作成。これをLLMへ渡し、ローカルデータの区画（Coherence）が、リファレンスデータ（Steam/MBZ）のどの範囲（開始〜終了インデックス）に該当するかという大局的なルーティングマップを生成。
  - このMap情報を用いてリファレンスデータを安全なサイズにスライス（Reduce）してからPhase 2へ送ることで、コンテキスト制限を回避。
- **Phase 2 (Sequential Mapping)**:
  - ローカルトラックをチャンク（最大20曲前後）に分割し、LLMにタグ付けと採番を指示。
...
- **`COMM` コメント欄の動的調整**: 
  - 既存フレームを `tags.clear()` で初期化後、既存の埋め込みコメント（あれば先頭に保持）に続けて `親ゲーム名, 親ゲームストアURL, [タグ1/ タグ2/ ...]` を `, ` で連結した単一の `COMM` フレームを書き込む。
  - ID3v2.3の推奨制限に準拠するため、UTF-16で 2000文字を超える場合は、情報の断片化を最小限に抑えつつ、末尾のタグ要素（`[...]` 内のコミュニティタグ）から `pop()` して調整する。
...
- **構造ベースのメタデータフォールバック**:
  タグの各項目は、複雑な優先度指定ループを排し、「トラック構造（番号・曲名）はSteam公式を絶対視し、アーティストや年など付加情報はMBZを優先する」という明確なルールで構築されます。該当ソースのトラックデータが存在しない場合のみ、次のソース（FINGERPRINT等）へ自動フォールバックされます。
- **監査レポートでの透明性確保**:
...
### 7.1 並行処理の動的制御 (Token Stingy Architecture)
- **Ollama環境時 (起動時最大スロット算出と固定コンテキスト)**: 
  - `nvidia-smi` によりシステムの総VRAMを自動検出し、プレフライトで実測したLLMモデルの基礎占有VRAMを差し引いた空き容量から、安全マージン（例：75%）を適用した「利用可能KVキャッシュ枠」を算出します。
  - その枠内に、固定された最大コンテキストサイズ（例: `32768`）のKVキャッシュがいくつ収まるかを計算し、最適な同時実行スレッド数（スロット数）を起動時に1度だけ自動決定します。以降はVRAMの都度計算を行わず、固定のスレッド数と固定の `num_ctx` で最速のパイプライン処理を実現します。
- **外部API環境時 (スレッドプールと適応型チャンク制御)**: 
  - VRAM計算はバイパスされ、設定された `LLM_LIMIT_RPM` とマシンのCPUコア数に基づいた適正なスレッド数へ自動で切り替わります。
  - リクエスト送信時、ペイロードに `max_tokens` (`LLM_CLOUD_MAX_TOKENS`) を明示的に付与し、さらに `LLM_LIMIT_TPM` (毎分トークン上限) を考慮して、429エラーを起こさない安全な範囲内でチャンクサイズを限界まで動的拡張します。
- 詳細な仕様については `docs/token_stingy.md` を参照してください。
...
分析は `tests/analyze_processing_results.py` で行い、以下を集計可能。

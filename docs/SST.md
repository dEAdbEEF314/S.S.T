...
5. **Package**: ZIP 化して Windows 側へ転送し、Python の `zipfile` ライブラリで展開後に ZIP を削除。

---

## 4. ディレクトリ構造

- `src/scout/`: コアアプリケーション・ロジック（モジュール化済み）。
...
5. **Package**: Created as a ZIP, moved to Windows, extracted via Python's `zipfile` library, and intermediate ZIP is deleted.

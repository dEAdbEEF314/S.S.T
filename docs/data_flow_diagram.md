...
3. **Processing**: 収集したデータを「Virtual Albums（仮想アルバム）」として規格化し、LLMに投入してトラックのマッピングと同一性スコア（Confidence）を推論させます。ここで `processor.py` による重複解消ロジックも介入します。

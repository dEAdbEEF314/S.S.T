...
SQLite (`sst_local_state.db`) に単一のキャッシュテーブルを追加し、外部API応答を永続化します。これにより、我々および相手先サーバー双方の負担を軽減します。

### 2.1 API 共通キャッシュ
- **テーブル名**: `api_cache`
- **キー**: `(service, query_key)` の複合キー
  - 例: `service=acoustid` + `query_key=fingerprint_hash`
  - 例: `service=mbz` + `query_key=query_hash`
- **バリュー**: 各APIの JSON レスポンス
- **有効期限 (TTL)**: 30日（`get_api_cache(service, key, ttl_days)` で制御）。

### 2.2 Steam PICS キャッシュ

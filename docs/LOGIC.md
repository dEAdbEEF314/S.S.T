...
次をすべて満たす場合、LLM を呼ばずに確定（決定論的 ARCHIVE 経路）します。

1. STEAM トラックリストが存在する。
2. フォーマット重複除外後のローカル曲数が STEAM 曲数と一致する。
3. 全ファイルが、トラック番号または正規化タイトルで一意に STEAM スロットへ対応する。
4. 対応は STEAM スロットと 1:1 で、未割当・欠落・STEAM 範囲外を残さない。
5. 同一トラック番号の複数フォーマット間で再生時間差が 1.0 秒未満である。

フォーマット重複除外の具体仕様:
- 同一の `(disc, track_number)` を持つファイル群、または正規化タイトル（アンダースコア・ピリオド・空白・大文字小文字の差異を吸収）が一致し再生時間差が 1.0 秒未満である群を「同一曲のフォーマット・バリアント束」として 1 曲に集約します。
...
LLM 実行・トークン制御ポリシー:
- **動的トークン予算計算**: `num_predict` はコンテキスト上限（`num_ctx`）からプロンプト評価トークン数を引いた残余容量を超えないよう安全にクランプする。
- **トランケーション即時フォールバック**: `done_reason in {"length", "max_tokens"}` が検知された場合、温度0での同一リトライは実行せず、即座にチャンクサイズを半減して再試行する。
- **出力軽量化**: `track_mapping` では長文の理由記述を抑え、必要最小限の ID スロットマッピングに特化してスループットを最大化する。
- **バックエンド**: `OLLAMA` はネイティブAPI、`GEMINI` と `OPENAI_COMPATIBLE` はHTTP API、`LITELLM` はLiteLLM Python SDKを使用する。LiteLLMは `LLM_MODEL` のプロバイダープレフィックスで接続先を選び、`LLM_BASE_URL=auto` なら標準エンドポイント、明示URLならカスタムAPIベースを使用する。全バックエンドの応答は同じJSON検証を通過し、接続・認証失敗を成功扱いしない。
...
アルバム共通アートワークの取得順は **EMBED → MBZ → STEAM** です。通常経路では選定済みの MBZ 候補を使います。FAST-TRACK では音源埋め込み画像がない場合に限って MBZ_SEARCH を遅延実行し、画像取得が失敗した場合は Steam 画像候補へ進みます。この APIC 専用検索は AcoustID のフィンガープリント、LLM、トラックのタグ選択に影響しません。
...
Steamストア説明文など外部由来のテキストは不信データとして扱う。説明文内の命令やプロンプトは実行せず、LLMは既存シグナルの整列だけを行う。LLM出力はJSON契約に限定し、`file_id`、`matched_v_idx`、スロット割当の参照範囲、重複、未割当、文字列長をコード側で検証する。検証不能な出力はReviewとする。
...
トラック照合および重複検査ではゼロ埋め正規化（`01` と `1` の差分吸収）を一貫して適用し、タグ出力値に干渉することなく誤判定を防止する。

### 4.3 FFmpegと中間生成物

FFmpegの非ゼロ終了、出力ファイル欠落・サイズ0、タイムアウト、変換例外は音声処理失敗（`audio_fail`）としてReviewへ送る。
stderrの既知警告（パケット損失や軽微なタイムスタンプ異常など）は音声品質警告（`audio_warn`）として分離検知し、本来Archive相当であっても警告付きReviewへ隔離し、監査レポートに警告対象トラック一覧を明示する。
通常のINFO運用では処理完了後に`SST_WORKING_DIR`の`final_<AppID>_*`と`buffer_<AppID>_*`を削除し、DEBUGまたは`--dev`では診断のため保持する。

### 4.4 Archive前の成果物再検証

Archive判定時は、`final_<AppID>_*`内の生成ファイルを再走査し、期待曲数・各ファイルの存在・サイズ・タグ（少なくともTIT2/TRCK/TPE1）・スロット完全充足を確認する（Preflight検証）。スロット突合にはゼロ埋め正規化契約を適用する。検証失敗時はZIPをArchiveへ保存せずReviewへ降格する。

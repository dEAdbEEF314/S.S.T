...
- **Ollama環境時 (動的VRAMセマフォ)**: 
  - `nvidia-smi` によりシステムの総VRAMを自動検出し、プレフライトで実測したLLMモデルの占有VRAMを差し引いた「空きVRAM（物理容量）の80%」を最大KVキャッシュ枠として定義します。
  - `Ollama /api/tokenize` により高精度に算出された要求VRAM容量が、この80%枠に収まる限りにおいて、動的にアルバム処理をディスパッチします。

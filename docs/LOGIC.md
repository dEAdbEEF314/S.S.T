...
5.  **信頼度閾値**:
    - **通常パス**: Identity Confidence >= 95, Integrity Quality >= 90
    - **STEAM-TRUST パス**: 物理同定（指紋）が欠落しているが Steam と構造一致する場合、品質閾値を **80%** へ動的に緩和し、物理データの欠落による減点を許容する。
...
### 6.1 ID3v2.3 仕様の強制
- **FFmpeg 変換**: MP3 (`-b:a 320k`), AIFF (`-write_id3v2 1`)。

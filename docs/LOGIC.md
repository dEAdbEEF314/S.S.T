...
未設定時のフォールバック値は `src/sst/config.py` に集約されており、`builder.py` や `llm.py` はそれを参照する。実行時には `Config.build_priority_config()` により `PriorityConfig` として束ねられ、トラック処理層から `MetadataBuilder` へ受け渡される。
...
- **ブレンド優先度とフォールバック**:
  タグの各項目は、`.env` で設定された個別の優先度（例: `PRIORITY_TIT2=MBZ,PICS_API...`）に従って最適なソースから採用されます。未設定時は `src/sst/config.py` の既定値が使われ、LLMが `use_fingerprint` などの指示を出した場合でも、該当ソースのトラックデータが存在しない場合は、次の優先度ソース（`PICS_API` など）へ自動フォールバックされます。
- **監査レポートでの透明性確保**:

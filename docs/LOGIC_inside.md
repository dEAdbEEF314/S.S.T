# S.S.T (Steam Soundtrack Tagger) 内部実装詳細仕様書 (実装視点)

本書は、既存の高レベル仕様書を排し、実際の Python ソースコードおよび設定ファイル (`.env.example`) から抽出された「実装の真実」を詳細に記述するものである。開発者が各モジュールの挙動を完全に理解し、デバッグや機能拡張を行うための究極のリファレンスとなることを目的とする。
...
- **Phase 2 (Sequential Mapping)**:
    ...
    - **マッピング指示**: `override_title`, `override_track`, `override_disc` の生成。
...
LLM の確信度に関わらず、以下の条件で強制的に `REVIEW` 判定：
1.  **Dirty/Conflicting Tags**: `^(\d+)([\s.-]+)` にマッチし、かつ小数点ではない、かつ実際のトラック番号と一致する、または `0` パディング/強いセパレータを伴う場合。
2.  **構造的異常**: `Track#0` の残存、同一ディスク内での `Duplicate Disc/Track pairs`、タイトルが "Unknown" のまま。
- **スマート重複解消ロジック (`_resolve_duplicate_mappings`)**:
    LLM が生成したインデックス重複を、以下の 3 段階で救済します。
    1.  **マルチディスク不整合の補正**: ローカルのディスク番号情報を使用して、Steamリスト内の正しいディスクセクションから該当する曲を再検索。
    2.  **名前ベースの再マッピング**: 重複グループ内の各ファイル名に対し、Fuzzy/Prefix マッチングを用いて Steam リストを再スキャンし、正しいインデックスをリカバリ。
    3.  **同名曲シーケンスの再配分**: Steam リスト内で同名曲が連続している場合、ローカルのファイル順序に従ってインデックスを順番に割り当て。
- **バリデーションと検閲層 (`validator.py`)**:
    - **重複タイトル検知**: アルバム内の 50% 以上のトラックで同一の曲名が使用されている場合（LLM誤認防止）。
...
- **`COMM` 最適化**: `tags.delall("COMM")` で初期化後、親ゲーム情報を ` | ` で連結。UTF-16で 2000文字を超える場合は、末尾の要素から `pop()` して調整。

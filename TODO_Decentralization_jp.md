...
### 3. サーバー C: Core, Storage & UI ノード（管理サーバー）
- **役割**: Prefect サーバー（状態管理、タスクキュー）、SeaweedFS（S3互換ストレージ）、および S.S.T Web UI (`ui` コンテナ) のホスティング。
- **要件**: サーバー A および サーバー B の両方からアクセス可能なIP、ならびにエンドユーザーがUIへアクセスするためのネットワーク経路を持つこと。
- **動作**: 常時稼働し、システム全体の状態やデータを管理しつつ、ユーザーインターフェースを提供する。
...
- [ ] **ステップ 5: UI コンテナのデプロイ設定**
  ...
  - 高速なZIP生成のため、UIの `.env` のS3接続先をローカルまたは内部ネットワークに向け、ポート8000を安全に公開する。

...
### 【高度な同定ヒント：AcoustIDボトムアップ分析】
各MusicBrainz候補の『Evidence』に注目してください：
- **ACOUSTID_RELEASE_MATCH**: このリリースIDは、ローカルの音声ファイルから生成された指紋が直接指し示した「物理的一致」です。
- **DIRECT_STEAM_LINK**: MusicBrainz上でこのリリースが対象のSteam AppIDと公式に紐付いています。
- **PUBLISHER_LABEL_MATCH**: Steamの販売元とリリースのレーベル名が一致しています。

これらが重複している候補は、アルバム名に多少の差異があっても『正解』である可能性が極めて高いです。
...
"""
        ...
        for i in range(0, len(track_ids), chunk_size):
            ...
            5. **ディスク番号の維持と補完 (重要)**:
               - **最優先ルール**: 提供された `local_context.disc` が 2 以上である場合、Steam公式データが Disc 1 しか持っていなくても、そのトラックは Disc 2 以降として扱うべきです。必ず `override_disc` に適切なディスク番号を出力しなさい。
               - ローカルのディスク番号が不明で、かつ Steam 公式トラックリストやコンテキストから明らかなディスク番号（例：Disc 2, 3等）が判明する場合も、その数値を `override_disc` に出力しなさい。
            6. **トラック番号の重複禁止 (絶対命令)**:
               - 同一ディスク内において、複数のトラックに同じ `override_track` を割り当てることは**システム的に禁止**されています。
               - **バリエーション曲（Inst, Remix, Extended等）の扱い**: 公式リストにオリジナル版（例：「Song A」）しかない場合、ローカルの「Song A (Inst.)」を同じ公式番号に割り当ててはいけません。バリエーション側は必ず `override_track: null` とし、個別のトラックとして独立させなさい。
               - **ファイル名重複への警告**: もし複数のローカルファイルが同じ番号（例：「01. A.mp3」と「01. B.mp3」）を持っており、それらを両方 `override_track: null` にすると、システム側で衝突が発生します。その場合は、適切な一意の番号を `override_track` で明示的に指定しなさい。
            7. **タイトルの一貫性**:
               - バリエーション曲の場合、`override_title` に "(Extended Ver.)" などの識別子を含めることで、他のトラックと区別できるようにしなさい。
            8. **最終チェック**:
               - あなたの出力する `track_instructions` 内で、同一ディスク内で `override_track` が重複していないか、送信前に必ずセルフチェックしなさい。

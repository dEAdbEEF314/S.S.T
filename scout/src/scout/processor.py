...
class LocalProcessor:
    def __init__(self, config: Any, db: DatabaseManager):
        ...
        self.llm = LLMOrganizer(
            ...
            user_language=config.user_language, force_local=config.llm_force_local
        )
    ...
    def process_album(self, app_id: int, install_dir: Path, steam_meta: SteamMetadata, on_track_complete: Optional[callable] = None) -> LocalProcessResult:
        ...
        # Act-14: Extract 'Internal Truth' from local tags to assist matching
        local_baseline = self._extract_local_baseline(track_groups)

        # 1. Evidence Gathering (NWO Hybrid Scoring)
    ...
    def _extract_local_baseline(self, track_groups: Dict) -> Dict[str, Any]:
        """Determines the 'representative truth' from existing local tags."""
        from collections import Counter
        albums, artists, years, track_names = [], [], [], []

        for (disc, title), variants in track_groups.items():
            track_names.append(title) # Logic title is already cleaned
            for v in variants:
                if v["meta"]:
    ...
    def _prepare_llm_track_context(self, track_groups: Dict) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            status, message, score, reason = self._validate_results(processed_tracks_meta, final_metadata, any_audio_failures, any_audio_warnings)
            ...
            self._send_notifications(app_id, steam_meta.name, status, message, score, reason)
            
            log_bundle = {"llm_log.json": llm_log, "mbz_log.json": mbz_log, "metadata.json": summary_meta,
                          "BASIS_for_CLASSIFICATION.md": self._generate_classification_basis(app_id, steam_meta, status, message, score, reason, len(processed_tracks_meta))}
            ...
            if not (self.config.env_mode == "development" and status == "error"):
    ...
    def _validate_results(self, tracks, llm_data, audio_fail, audio_warn) -> Tuple[str, str, int, str]:
        """Applies SST.md 2.2 and 2.3 strict gating rules."""
        status, message = "archive", "Success"
        first_tid = next(iter(llm_data)) if llm_data else None
        score = llm_data.get(first_tid, {}).get("confidence_score", 0) if first_tid else 0
        reason = llm_data.get(first_tid, {}).get("confidence_reason", "No data") if first_tid else "N/A"
        
        # Anomalies
        z_count = sum(1 for t in tracks if t["tags"]["track_number"] == "0")
        u_count = sum(1 for t in tracks if t["tags"]["title"] == "Unknown")
        dirty_pattern = re.compile(r'^\d+\s*[-.]\s*')
        d_count = sum(1 for t in tracks if dirty_pattern.match(t["tags"]["title"]))
        ...
        if z_count > 0 or u_count > 0 or d_count > 0:
            ...
            label = llm_data.get(first_tid, {}).get("semantic_label") if first_tid else None
            message = f"{label} [{', '.join(issues)}]" if label else f"Metadata anomalies: [{', '.join(issues)}]"
        ...
        if status == "archive" and score < 95:
            status, message = "review", f"Low confidence ({score}%): {reason}"

        return status, message, score, reason

    def _send_notifications(self, app_id, name, status, message, score, reason):
        fields = [{"name": "AppID", "value": str(app_id), "inline": True}, {"name": "Status", "value": status.upper(), "inline": True},
                  {"name": "Confidence", "value": f"{score}%", "inline": True}, {"name": "Reasoning", "value": reason[:1024], "inline": False}]
    ...
    def _generate_classification_basis(self, app_id, steam_meta, status, message, score, reason, count):
        ...
        return dedent(f"""\
            ...
            - **Status**: {status.upper()} | **Conf.**: {score}/100
            - **Reason**: {message}
            - **LLM**: {reason}
            - **Parent**: {steam_meta.parent_name or 'N/A'} ({steam_meta.parent_app_id or 'N/A'})
            - **Tracks**: {count}
        """)

    # --- Helper methods (kept for now but could be further separated) ---
    ...
    def _group_by_logical_track(self, files: List[Path]) -> Dict[Tuple[int, str], List[Dict[str, Any]]]:
        ...
        for f in files:
            ...
            disc = int(str(meta.get("disc_number", 1)).split('/')[0]) if meta.get("disc_number") else 1

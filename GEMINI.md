...
- **Environment**: Always use `uv run` for script execution. Ensure `PYTHONPATH` includes `scout/src`.
- **Logic Priority**: Archive reliability is paramount. If metadata is ambiguous, route to `Review`.
- **Change Files**: If you add or modify any files, be sure to update CHANGE_HISTORY.md with the following information: “Date and time (YYYY/MM/DD hh:mm:ss), name of the modified file, and details of the changes (in Japanese).”

## 📂 Directory Layout

- `scout/src/scout/`: Core application logic.
